...
export const StatsView: React.FC = () => {
    const { stats } = useSocketStore();

    // In a real app, 'change' would be calculated from previous period
    const statItems = [
        { label: 'Total Calls', value: stats.callsToday.toString(), change: stats.callsTrend, icon: Phone, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
        ...
        <div className="flex flex-col h-full p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-3">
                    <button className="px-4 py-2 rounded-lg bg-zinc-800 text-zinc-300 text-sm font-medium hover:bg-zinc-700 transition-colors">Last 7 Days</button>
                    <button className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-500 transition-colors shadow-lg shadow-indigo-600/20">Download Report</button>
                </div>
            </div>
                    ...
                    <div className="flex-1 flex items-end justify-between gap-4">
                        {hourlyData.map((d, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-4 group">
                                <div
                                    className="w-full rounded-t-lg transition-all duration-500 group-hover:brightness-110 bg-gradient-to-t from-indigo-600/50 to-indigo-500"
                                    style={{
                ...
                <div className="bg-zinc-900 border border-white/5 rounded-2xl p-8 flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        {[
                            ...
                            { label: 'Follow Up', value: 32, color: 'bg-indigo-500' },
